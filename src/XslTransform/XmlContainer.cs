...
using System.IO;
using System.Xml;
using XslTransform.Abstractions;
using XslTransform.Models;

namespace XslTransform
{
    public class XmlContainer : ConcurrentDictionary<string, XmlTreeDocument>, ITemplatesContainer<XslTemplateRequest, string, XmlTreeDocument>
    {
        ...
        public void Load(params XslTemplateRequest[] args)
        {
            ...
            foreach (var request in args)
            {
                if (request.FileFormat.Equals(XmlFormat))
                {
                    ...
                    {                        
                        this.AddOrUpdate(request.Name,
                            new XmlTreeDocument(request.Name, request.CultureCode, LoadDoc(filePath)),
                            (k, o) =>
                            {
                ...
                else if (request.FileFormat.Equals(XsltFormat))
                {
                    ...
                    {                        
                        this.AddOrUpdate(request.Name,
                            new XmlTreeDocument(request.Name, LoadDoc(filePath)),
                            (k, o) =>
                            {
